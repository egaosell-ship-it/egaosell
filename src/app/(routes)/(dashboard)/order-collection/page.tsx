...
export default async function OrderCollectionPage({ searchParams }: PageProps) {
  ...
  if (user) {
    if (!businessId) {
      ...
      if (mainBiz) {
        redirect(`/order-collection?businessId=${mainBiz.id}`);
      }
    } else {
  ...
  return (
    <div className="flex flex-col h-full gap-4">
      <PageHeader 
        title={businessName ? `${businessName} 주문 수집` : "주문 수집"} 
        description="보유 스토어의 주문 데이터를 수집합니다." 
      />
          ...
          {ownedStores.length === 0 ? (
            ...
            ownedStores.map(store => {
              ...
              return (
                <Link
                  key={store.id}
                  href={`/order-collection?businessId=${businessId}&storeId=${store.id}`}
                  style={{ 
      ...
      {!hasBusinessId ? (
        <div className="mt-2 flex flex-col gap-4">
          <Panel>
            <div className="flex flex-col items-center justify-center py-12 text-on-surface-variant">
              ...
              <p className="text-sm font-medium">상단 메뉴 [주문 수집]에서 상호(사업자)를 선택해주세요.</p>
            </div>
          </Panel>
        </div>
      ) : (
        <div className="flex-1 flex flex-col mt-2 min-h-[400px]">
          <div className="flex items-center gap-2 mb-3 px-1">
            ...
            <span className="text-xs text-on-surface-variant">스토어 주문 데이터 수집</span>
          </div>
          ...
          <div className="flex-1 flex flex-col border border-outline-variant rounded-md overflow-hidden shadow-sm" style={{ backgroundColor: hexToRgba(currentColor, 0.1) }}>
            ...
            <div className="border-t border-outline-variant p-3 flex justify-end shrink-0" style={{ backgroundColor: 'rgba(255, 255, 255, 0.5)' }}>
              <Button icon="transform">수집</Button>
            </div>
          </div>
        </div>
      )}
