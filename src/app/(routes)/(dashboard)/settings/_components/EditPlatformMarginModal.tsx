...
import { updatePlatformMarginAction } from "@/app/actions/platformMargin.actions";
...
interface Props {
  ...
  margin: PlatformMarginProps;
}

export default function EditPlatformMarginModal({ businesses, margin }: Props) {
  const [isOpen, setIsOpen] = useState(false);
  const [mounted, setMounted] = useState(false);
  const [state, formAction, isPending] = useActionState(updatePlatformMarginAction, null);
    ...
    <div className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center p-4">
      <div className="bg-surface-container-lowest rounded-xl shadow-lg w-[90vw] sm:w-[450px] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-outline-variant flex justify-between items-center bg-surface shrink-0">
          <h3 className="text-lg font-bold text-on-surface whitespace-nowrap">플랫폼 마진 세팅 수정</h3>
        ...
        <form action={formAction} className="flex flex-col overflow-hidden h-full">
          ...
          <div className="p-6 flex flex-col gap-4 overflow-y-auto max-h-[65vh]">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <select name="businessId" required defaultValue={margin.businessId} className="border border-outline-variant rounded p-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none bg-surface transition-all w-full cursor-pointer">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <div className="flex items-center gap-2">
                <input type="color" name="colorCode" defaultValue={margin.colorCode || "#E2E8F0"} className="w-10 h-10 border-0 p-0 rounded cursor-pointer bg-transparent" />
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="number" step="0.01" name="commissionRate" required defaultValue={margin.commissionRate} className="border border-outline-variant rounded p-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none bg-surface transition-all w-full" placeholder="예: 10.5" />
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-on-surface-variant whitespace-nowrap">배송비 (원)</label>
              <input type="number" name="shippingFee" required defaultValue={margin.shippingFee} className="border border-outline-variant rounded p-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none bg-surface transition-all w-full" placeholder="예: 3000" />
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-on-surface-variant whitespace-nowrap">기타비용 (광고비, 고정비, 세금 등 - 원)</label>
              <input type="number" name="otherCosts" required defaultValue={margin.otherCosts} className="border border-outline-variant rounded p-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none bg-surface transition-all w-full" placeholder="예: 500" />
            </div>
