...
import { createOwnedStoreAction } from "@/app/actions/ownedStore.actions";
...
export default function AddOwnedStoreModal({ businesses, margins }: Props) {
  const [isOpen, setIsOpen] = useState(false);
  const [mounted, setMounted] = useState(false);
  const [state, formAction, isPending] = useActionState(createOwnedStoreAction, null);
    ...
    <div className="fixed inset-0 bg-black/50 z-[9999] flex items-center justify-center p-4">
      <div className="bg-surface-container-lowest rounded-xl shadow-lg w-[90vw] sm:w-[450px] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200">
        ...
        <div className="px-6 py-4 border-b border-outline-variant flex justify-between items-center bg-surface shrink-0">
          <h3 className="text-lg font-bold text-on-surface whitespace-nowrap">새 스토어 URL 입력</h3>
        ...
        <form action={formAction} className="flex flex-col overflow-hidden h-full">
          <div className="p-6 flex flex-col gap-4 overflow-y-auto max-h-[65vh]">
            ...
            <div className="flex flex-col gap-1.5">
              ...
              <input type="text" name="loginId" required className="border border-outline-variant rounded p-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none bg-surface transition-all w-full" placeholder="로그인아이디를 입력하세요" />
            </div>

            <div className="flex flex-col gap-1.5">
              <label className="text-xs font-semibold text-on-surface-variant whitespace-nowrap">사이트명</label>
              <input type="text" name="siteName" required className="border border-outline-variant rounded p-2.5 text-sm focus:border-primary focus:ring-1 focus:ring-primary outline-none bg-surface transition-all w-full" placeholder="사이트명을 입력하세요 (예: 쿠팡윙)" />
            </div>
  ...
  return (
    <>
      <Button icon="add" onClick={() => setIsOpen(true)}>새스토어URL입력</Button>
