...
export async function testNaverApiAction() {
  try {
    ...
    // 사용자 요청 조건: app_secret이 'Qfnt57DVAJSsHrgrY35sH' 인 데이터 조회
    ...
    if (error || !naverApi) {
      return { 
        success: false, 
        error: `Database Error: Could not find Naver API credentials for the secret 'Qfnt57DVAJSsHrgrY35sH'. (${error?.message || 'Not found'})` 
      };
    }
